...
var scotchApp = angular.module('scotchApp', [ 'multi-select', 'fileAppDir',
		...
		'PropertySearchServices', 'ajaxService', 'PropertyServices', 'blockUI',
		'alertsService', 'ui.bootstrap', 'propertyControllers',
...
scotchApp
		.config(function($stateProvider, $urlRouterProvider) {
			...
			/*
			 * .state('requirement', { url : '/requirement', templateUrl :
			 * 'pages/postRequirement.html', controller :
...
scotchApp.controller('loginController', function($scope, $http, loginService,
		$location, $rootScope) {
	$scope.formData = {};
	// process the form
	$scope.processForm = function() {
...
/*
 * function GenericController($scope, loginService){ $scope.getSuccessLogin =
 * function(response) { return "success"; } $scope.getErrorLogin =
 * function(response) { alertsService.RenderErrorMessage("error in reqiuest"); }
 * $scope.getSuccessRegister = function(response) {
 * loginService.login($scope.formData, $scope.getSuccessLogin,
 * $scope.getErrorLogin); } $scope.getErrorRegister = function(response) {
 * alertsService.RenderErrorMessage("error in reqiuest"); } $scope.register =
 * function() { loginService.registration($scope.formData,
 * $scope.getSuccessRegister, $scope.getErrorRegister); } };
 */
scotchApp
		.controller(
				...
				function($scope, $http, loginService) {
					$scope.formData = {};
					// $injector.invoke(GenericController, this, {$scope:
					// $scope});
					...
					$scope.signup = function() {
						...
						$scope.getErrorRegister = function(response) {
							alertsService
									.RenderErrorMessage("error in reqiuest");
						}
						loginService.registration($scope.formData,
								$scope.getSuccessRegister,
								$scope.getErrorRegister);
					};
				});
														...
														.forEach(
																...
																function(da) {
																	if (($scope.formData.bedroom != null || $scope.formData.bedroom != undefined)
																			&& !(angular
																					.equals(
																							$scope.formData.bedroom[0].label,
																							da.label) || angular
																					.equals(
																							$scope.formData.bedroom[1].label,
																							da.label))) {
																		da.disabled = true;
																	}
																});
														...
														.forEach(
																...
																function(da) {
																	if (($scope.formData.budget != null || $scope.formData.budget != undefined)
																			&& !(angular
																					.equals(
																							$scope.formData.budget[0].label,
																							da.label) || angular
																					.equals(
																							$scope.formData.budget[1].label,
																							da.label))) {
																		da.disabled = true;
																	}
																});
						...
						$http(
								...
								.success(
										function(data) {
											...
											searchService
													.saveSearchResponse(data.searchProperties);
...
scotchApp.controller('latestSearchCntrl', function($scope, propertyService,
		GalleryImageService, alertsService, $modal) {
	...
	$scope.openModal = function(propertyId) {
		...
		propertyService.findPropertyImages(property,
				$scope.findImagesCompleted, $scope.findImagesError);
...
scotchApp
		.controller(
				...
				function($scope, $http, propertyService, loginService,
					...
					$scope.processForm = function() {
						...
						$scope.postProperty = function(response) {
							...
							if (null != $scope.files) {
								...
								imageService.uploadImages($scope.formData,
										$scope.imageUploadSuccessFull,
										$scope.error)
							}
						}
						...
						if ($scope.formData.registrationInfo.newUser == 'false') {
							loginService.login($scope.formData,
									$scope.postProperty, $scope.error);
						} else {
							loginService.registration($scope.formData,
									$scope.postProperty, $scope.error);
						}
					...
					$scope
							.$watch(
									'formData.propertyFeatureInfo.propertyMandateInfo.propertyTypeId',
									function(selectedVal) {
										if (selectedVal !== undefined) {
											// Variables displaying single
											// fields
