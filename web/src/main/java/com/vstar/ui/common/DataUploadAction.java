...
public class DataUploadAction extends ActionSupport implements ServletRequestAware
{
  ...
  private void loadData(List<DataRowBean> totalRows) throws Exception
  {
    for (DataRowBean row : totalRows)
    {
      ...
      {
        propertyMandateInfo.setPropertyTypeId(PropertyTypeEnum.valueOfString(row.getPropertyType())
            .getId());
      }
      // TODO: set location
      ...
      {
        propertyMandateInfo.setPurchaseType(PropertyPurchaseTypeEnum.valueOfString(
            row.getPurchaseType()).getId());
      }
  ...
  private long formatMoney(String moneyInput)
  {
    ...
    if (!ValidationUtil.isNullEmpty(moneyInput))
    {
      ...
      if (moneyInput.toLowerCase().contains("cr"))
      {
        double cr = Double.valueOf(moneyInput.toLowerCase().replace("cr", "").replace(",", "").replace(" ", ""));
  ...
  // generate city id based on state .If state is not exist create new state and
  // city
  private long createCity(String stateName, String cityName)
  {
    long cityId = 0;
    if (!ValidationUtil.isNullEmpty(stateName) && !ValidationUtil.isNullEmpty(cityName))
    {
      PropStateDao propStateDao = propStateDaoProcess.getPropStateDaoByName(stateName);
      if (propStateDao == null)
      {
        propStateDao = new PropStateDao(0, null, stateName, new Date());
        propStateDao = propStateDaoProcess.addUpdatePropStateDao(propStateDao);
      }
      if (propStateDao != null)
      {
        PropCityDao propCityDao = propCityDaoProcess.getPropCityByStateIdCityName(
        ...
        {
          propCityDao = new PropCityDao(0, propStateDao, cityName, new Date());
          propCityDao = propCityDaoProcess.addUpdatePropCityDao(propCityDao);
          cityId = propCityDao.getPropCityId();
        }
      }
    }
  ...
  private boolean checkExcelSheetHeaders()
  {
    ...
    for (int x = 0; x < ExcelImportData.MAX_COL_NUM; x++)
    {
      ...
      String header = currentCell.getStringValue().trim();
      switch (x)
      {
        case 0 : {
